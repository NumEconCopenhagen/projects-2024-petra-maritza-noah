...
class InterpolationSolver:
    ...
    #     X = rng.uniform(size=(100, 2))  # Increase the number of points
    #     y_points = [(0.2, 0.2), (0.8, 0.2), (0.8, 0.8), (0.5, 0.5)]

    #     # Instantiate the solver
    #     solver = InterpolationSolver(X, None)
    ...
    #         if solver.A is None or solver.B is None or solver.C is None or solver.D is None:
    #             print("Cannot form the required quadrilateral. Return NaN.")
    #             f_y_approx = np.nan
    #             f_y_true = np.nan
    #         else:
    ...
    #             r_ABC = solver.compute_barycentric_coordinates(solver.A, solver.B, solver.C)
    #             r_CDA = solver.compute_barycentric_coordinates(solver.C, solver.D, solver.A)

    #             inside_ABC = all(0 <= r <= 1 for r in r_ABC)
    #             inside_CDA = all(0 <= r <= 1 for r in r_CDA)

    #             if inside_ABC:
    #                 f_y_approx = r_ABC[0] * solver.f(solver.A) + r_ABC[1] * solver.f(solver.B) + r_ABC[2] * solver.f(solver.C)
    #             elif inside_CDA:
    #                 f_y_approx = r_CDA[0] * solver.f(solver.C) + r_CDA[1] * solver.f(solver.D) + r_CDA[2] * solver.f(solver.A)
    #             else:
    #                 f_y_approx = np.nan

    #             f_y_true = solver.f(solver.y)
    ...
    #     A, B, C, D = solver.A, solver.B, solver.C, solver.D
