...
class InterpolationSolver:
    ...
    def set_y(self, y):
        """
        Set the value of y for which to find nearest points and compute barycentric coordinates.
        """
        self.y = y
    
    def find_nearest_points(self):
        """
        Find the nearest points A, B, C, D in X relative to y.
        """
    ...
    def interpolate_value(self):
        """
        Interpolate the value of f(y) based on the nearest points A, B, C, D and computed barycentric coordinates.
        """
        if self.A is None or self.B is None or self.C is None or self.D is None:
            return np.nan
        ...
        if all(0 <= r <= 1 for r in r_ABC):
            interpolated_value = r_ABC[0] * self.f(self.A) + r_ABC[1] * self.f(self.B) + r_ABC[2] * self.f(self.C)
            return interpolated_value, 'ABC'
        elif all(0 <= r <= 1 for r in r_CDA):
            interpolated_value = r_CDA[0] * self.f(self.C) + r_CDA[1] * self.f(self.D) + r_CDA[2] * self.f(self.A)
            return interpolated_value, 'CDA'
        else:
            return np.nan, 'None'
    ...
    def solve_question1(self):
        ...
        Solve Question 1: Find A, B, C, D and print values.
        """
        self.A, self.B, self.C, self.D = self.find_nearest_points()
